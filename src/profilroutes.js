...
// 1. Hantera default-avatar först
...
// 2. Hantera andra vanliga avatar/theme equips
router.post("/profile/equip/:id", requireLogin, async (req, res) => {
  const pool = req.app.locals.pool;
  const userId = req.session.user.id;
  const itemIdRaw = req.params.id;

  try {
    const itemId = parseInt(itemIdRaw);
    if (isNaN(itemId)) return res.redirect("/profile");
...
router.get("/profile", requireLogin, async (req, res) => {
  ...
  try {
    ...
    for (const q of completed) {
      history.push({ title: q.title, type: q.type, date: q.completed_at, status: "completed" });
    }
    for (const q of missed) {
      history.push({ title: q.title, type: q.type, date: q.due_at, status: "missed" });
    }
    ...
    if (user.avatar && user.avatar.startsWith("shop/")) {
      const [rowsA] = await pool.query("SELECT id FROM shop_items WHERE CONCAT('shop/', image) = ?", [user.avatar]);
      if (rowsA.length > 0) equippedAvatarId = rowsA[0].id;
    }

    if (user.theme_bg) {
      const [rowsT] = await pool.query("SELECT id FROM shop_items WHERE image = ?", [user.theme_bg]);
